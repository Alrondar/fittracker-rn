...
3. **`copy_program_for_user` НЕ копирует `target_rpe` и `progression_policy`** (только exercise_id/name/sets/reps_range/rest/intensity/position). Не строить программу, где target_rpe — единичное отличие фаз.
...
6. **Валидация** (обязательна, до отчёта): `references/validate.sql` — все 5 запросов чисто; затем E2E: `copy_program_for_user('<id>','<auth.uid>')` внутри `begin; … rollback;` (техника set_config из памяти проекта) и сравнить counts оригинала/копии; проверить что у копии все дни имеют `phase_id`.
...
- `references/validate.sql` — 7 проверочных запросов (резолв имён, структура фаз/дней, conformance, diff_counts, E2E-копия, целостность ссылок, баланс жим/тяга).
