...
Срез: 24.09.2026 (main) [верификация 6 функций после аудитов, критичные фиксы FIX-1…3]
...
| FD-4 | 🔴 | 🔲 | **BLOCKED (нужна авторизация)**: `progression_policy` теряется в snapshot/start. Требует миграции Supabase (колонка `workout_exercises.progression_policy`) + server-side RPC `save_program_snapshot` (отсутствует в репо). Не трогать shared DB без явного разрешения |
| FD-P2 | 🟡 | 🔲 | Группа средних: (FD-11) alternatives/ACWR/deload-логика/warmup/cycle-учёт; (FD-12) session/nutrition/units — мелочи. Не начаты |
...
| VF-P1 | 🟠 | 🔲 | Неисправленные находки верификации: (VF-1) early-return `updateSetFeedback` (useWorkoutSession.ts:299) сравнивает только rpe/rir/difficulty → патч только `isWarmup` на пустом сете отбрасывается; (VF-2) select `recentLogs` (workoutService.ts:354) без `reps_left/reps_right` → previous для unilateral = null → NO_HISTORY; (VF-3) `saveWorkout` пишет `finished_at` даже когда `flushPendingLogs` проглотил ошибки; (VF-4) auto-add warmup-сетов не синхронизирует `workout_exercises.target_sets` — сеты исчезают из UI после перезахода; (VF-5) `is_estimated_reps` сбрасывается при пересохранении упражнения; (VF-6) `getPersonalRecordsWithDates` без FD-7-фильтра (skipped/незавершённые в PR); (VF-7) `MuscleStatsSection.rowsToMuscleLoad` без дедупли front∩back → двойной счёт мышцы, расхождение с Workout Report; (VF-8) >7 мест хардкода «кг» в обход `useWeightDisplay` (остаток DA-P1); (VF-9) `['todayRecovery']` не инвалидируется после чек-ина; (VF-10) `CycleCheckInSheet.tsx:28` defaultDate через `toISOString()` (UTC) — нарушение FD-5 |
