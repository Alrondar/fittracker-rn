...
| ENG-16 | 🔴 | ✅ | Fix progression pollution on temporary replacement: update `workout_exercises.exercise_id` in DB during temp replace, so logs are attributed to the actually performed exercise, not the original one. |
