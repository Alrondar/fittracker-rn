...
| H-MUSCLE-2 | 🟠 | 🟡 | Progress hub: новая секция «Мышцы» (CI-4 Muscle Volume Analysis расширен до отдельной UI-зоны) с 3 вкладками: **Нагрузка** (периоды 7/30/90/всё + карта + легенда), **Усталость** (карта по бакетам 0–2/3–5/6–14/15+ дней + список мышц с днями), **Сила** (карта по сохранению e1RM + топ-5 упражнений). Источники: `muscleStatsService` (единственный Supabase-запрос) + `useMuscleStats` (React Query, staleTime 5 мин). Статус 🟡 — реализовано, ожидает проверки на устройстве (tsc/eslint не запускались в этой сессии). |
