...
Срез: 05.08.2026. Статусы задач → TASKS_STATUS.md. Инвентарь → FILE_INVENTORY.md. Детальный рефакторинг → refactoring_guide.md.
...
app/
 ...
 src/
   ...
   components/workout/      # ExerciseCard, ExerciseSlider, SetsGrid, SetFeedbackControl, WarmupBlock, RestTimer, WorkoutTimer
   ...
   types/                   # database.types.ts (UTF-8, синхронизирован)
   utils/                   # errorMapper.ts, macroCalculator.ts, rpe.ts
...
| useWorkoutSession.ts|Сессия тренировки (SEC-2: updateSet локальный, FEAT-7: updateSetFeedback)|
...
daily_readiness: auth.uid() = user_id (ALL)
pain_events: auth.uid() = user_id (ALL)
...
Все новые RPC — в supabase/migrations под тем же ревью, что и клиентский код
...
RPC-3 ✅ закрыт 01.08.2026, обновлён FEAT-7 05.08.2026
...
useTimerSettings: поле autoStartRest для FEAT-1.2 (05.08.2026)

workout_logs: колонки rpe (smallint 1–10), rir (smallint 0–5), difficulty (text: easy/moderate/hard/max), created_at (timestamptz) — добавлены FEAT-7 (05.08.2026)
daily_readiness: таблица для daily readiness check-in (FEAT-1.8, миграция применена 05.08.2026)
pain_events: таблица для pain flag (FEAT-1.9, миграция применена 05.08.2026)
...
| Файл|Проблема|
| ---|---|
| badge.ts / button.ts / common.ts / dashboard.ts / history.tsx|Разное|
...
Reanimated: НЕ читать .value в JSX-рендерере — только внутри useAnimatedStyle/worklet
Gesture Handler: использовать simultaneousWithExternalGesture для разрешения конфликтов со ScrollView
...
Ключи LLM-провайдеров на клиенте (только через Edge Functions)
AI-рекомендации, обходящие safety-ограничения (injury_exercise_warnings — хард-фильтр; AI — советник)
Отправка PII/деталей фармакологии в LLM без явного согласия пользователя
...
Подтвердить версию файла. Если файл прислан в двух вариантах или в refactoring_guide.md есть пометка «разнобой версий» — спросить пользователя, какая версия актуальна на диске. НЕ угадывать.
Grep по импортам. grep -r "имя_файла" src/ app/ — найти всех потребителей. Изменение экспорта = проверка всех импортов.
Проверить мёртвость. Если файл в списке SCALE-3 (useActiveProgram.ts, AnimatedButton, SwipeableCard, BottomSheet.tsx, Frame*.svg) — сначала подтвердить мёртвость grep'ом, потом удалить.
Проверить лимит строк. Если файл > 450 строк — отказаться от добавления функционала, предложить разбиение (SCALE-5).
Проверить tech debt. Если файл в FILE_INVENTORY.md помечен как tech debt — не усугублять долг.
Проверить database.types.ts. Если задача касается новой RPC — предупредить, что types рассинхронизированы; вызов делать строковый.
Проверить RLS. Если задача касается новой таблицы — убедиться, что она есть в списке RLS.
...
ROADMAP.md — план разработки и дорожная карта (продуктовые и технические этапы)
TASKS_STATUS.md — сводная таблица всех задач со статусами (новые фичи получают ID FEAT-N)
FILE_INVENTORY.md — инвентарь файлов проекта с назначением и пометками долга
PROMPTS.md — доп. промты (карта зависимостей, RPC-правила, дизайн-система)
refactoring_guide.md —  заморожен 04.08.2026 (архив аудита; не обновлять)
