...
✅ Тип активности «Активация» (ЗАВЕРШЕНО): колонка can_be_activation в exercises (62 упражнения, критерий 🅳: оборудование «резинки/эспандеры» + ключевые слова «резинк/мобил/актив/ротаци»); RPC search_exercises возвращает can_be_activation + параметр activation_filter; бейдж «⚡ Активация» в справочнике (ExerciseRow) + фильтр-переключатель «Только активация» (useExercises.activationOnly); бейдж в детальном экране упражнения (hero); бейдж «Активация/Растяжка» в разминке (WarmupBlock, по can_be_activation).
🔲 Фото прогресса: привязка фото к замерам тела (Supabase Storage).
✅ Нечёткий поиск (ЗАВЕРШЕНО): pg_trgm + word_similarity (порог 0.4) в RPC search_exercises;
   гибридный поиск LIKE (точные подстроки) + word_similarity (опечатки «гантеля»→«гантели»,
   «присидания»→«приседания»); GIN-индекс idx_exercises_name_norm_trgm на нормализованном
   названии; ранжирование по релевантности (точные совпадения выше нечётких).
