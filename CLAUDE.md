...
Транзакция
Идемпотентность
copy_program_for_user(p_program_id, p_user_id)
Копирование программы с фазами/днями/упражнениями
DEFINER + проверка auth.uid()
✅ неявная
✅ проверка source_program_id
create_workouts_for_program(p_user_id, p_program_id)
Создание всех тренировок программы upfront
DEFINER + проверка auth.uid()
✅ неявная
✅ проверка существования
generate_share_code(p_program_id)
Генерация уникального кода FIT-XXXXXX
DEFINER + проверка владельца
✅ неявная
✅ (повторная генерация допустима)
search_exercises(...)
Нечёткий поиск + фильтры + пагинация
DEFINER, STABLE
n/a (read-only)
n/a
update_day_position / update_exercise_position
Обновление позиции (drag & drop)
⚠️ нет явного INVOKER (защита через RLS)
✅
✅
handle_new_user
...
handle_workouts_updated_at / update_updated_at_column
Триггеры updated_at
INVOKER
✅
n/a
normalize_equipment_* / split_equipment* / migrate_exercise_equipment
Миграция/нормализация данных оборудования
INVOKER
✅
✅ (ON CONFLICT DO NOTHING)

✅ Подтверждённые сильные стороны
Транзакционная целостность: copy_program_for_user и create_workouts_for_program — единые PL/pgSQL-блоки; любой сбой откатывает всю операцию.
Идемпотентность: обе функции создания имеют явные проверки IF EXISTS и не дублируют данные.
Безопасность: модифицирующие функции с SECURITY DEFINER явно проверяют auth.uid() (RAISE EXCEPTION при чужом user_id).
Батчинг: create_workouts_for_program вставляет упражнения через INSERT INTO ... SELECT (нет N+1 на уровне упражнений).
Нечёткий поиск: search_exercises корректно использует pg_trgm (word_similarity > 0.4), нормализацию ё→е, ранжирование по релевантности + популярности.
⚠️ Замечания по серверной логике (задачи)
RPC-1 🟢: update_day_position / update_exercise_position не имеют явного SECURITY INVOKER. Защита обеспечивается RLS таблиц, но рекомендуется добавить явное объявление для защиты от будущих изменений настроек БД.
...
beginner
3
Адаптация (4) → Прогрессия (3) → Дилоуд (1)
StrongLifts 5×5
beginner
2
База 5×5 (8) → Интенсификация (3) → Дилоуд (1)
PPL Классический
intermediate
3
Гипертрофия (4) → Сила (3) → Дилоуд (1)
Upper/Lower
intermediate
4
Гипертрофия (4) → Сила (3) → Дилоуд (1)
...
SEC-1 — Service Role Key закоммичен в .env 🔴
Проблема: В .env закоммичен SUPABASE_SERVICE_ROLE_KEY — ключ, обходящий все RLS-политики.
Действие:
Ротировать ключ в Supabase Dashboard немедленно
Вычистить из git-истории (git filter-repo/BFG)
Добавить .env в .gitignore
Хранить секреты через EAS Secrets/CI vault
Коммитить только .env.example с плейсхолдерами
...
SEC-3 — Вложенный VirtualizedList без scrollEnabled={false} 🟠
Проблема: src/components/program/DayCard.tsx — DraggableFlatList без scrollEnabled={false} внутри ScrollView.
Действие: Добавить scrollEnabled={false} в DayCard.tsx.
SEC-4 — .single() на потенциально пустой выборке 🟠
Проблема: programsService.ts → createWorkoutsFromProgram — .single() бросит ошибку, если совпадений 0.
Действие: Заменить на .maybeSingle() или удалить функцию, если не используется (канонический путь — RPC create_workouts_for_program).
SEC-5 — Разное поведение сброса пароля 🟠
Проблема: app/profile/settings.tsx → handleChangePassword вызывает supabase.auth.resetPasswordForEmail(email) напрямую, без redirectTo.
Действие: Вызывать через authService.sendPasswordReset(email, 'fittracker://reset-password').
...
SEC-10 — Прямые вызовы supabase.* в UI 🟢
Проблема: app/profile/settings.tsx напрямую делает supabase.from(...) и supabase.auth.*.
Действие: Вынести в services/.
...
Проблема: .env, app.json → extra, хардкод в src/lib/supabase.ts.
Действие: Единый источник истины.
...
Результаты: транзакционность и идемпотентность критичных RPC подтверждены; найдены замечания RPC-1 (явный SECURITY INVOKER), RPC-2 (дубли RLS — закрыто), RPC-3 (нужен upsert_workout_logs).
...
RPC-2: Очистка дублирующихся RLS-политик (34 → 12, скрипт supabase/cleanup_duplicate_policies.sql) — ВЫПОЛНЕНО 29.07.2026
SCALE-6: Аудит серверной логики RPC — ВЫПОЛНЕНО 29.07.2026
SEC-1: Ротировать и удалить service_role key
SEC-3: Починить scrollEnabled={false} в DayCard.tsx
SEC-4: .single() → .maybeSingle() в createWorkoutsFromProgram
SEC-5: Сброс пароля из Settings через authService с правильным redirectTo
SEC-10: Вынести supabase.* из settings.tsx в services/
RPC-1: Добавить явный SECURITY INVOKER в update_day_position / update_exercise_position
Реализовать или скрыть 3 "фейковых" тумблера (напоминания, имперские единицы) и кнопку "Экспорт"
...
✅
Перенесено в useProgramEditor, DraggableFlatList
📊 Статус задач
...
Найдены замечания: RPC-1 (явный SECURITY INVOKER), RPC-3 (нужен upsert_workout_logs).
...
🔴 SEC-1: Ротировать service_role key
🔴 SEC-2: Автосохранение подходов тренировки
🟠 SEC-3: scrollEnabled={false} в DayCard.tsx
🟠 SEC-4: .single() → .maybeSingle() в createWorkoutsFromProgram
🟠 SEC-5: Сброс пароля из Settings через authService
...
🟢 SEC-10: Вынести supabase.* из settings.tsx
🟢 RPC-1: Явный SECURITY INVOKER в функциях обновления позиций
...
Последнее обновление: 29.07.2026 (аудит SCALE-6 выполнен, RLS-политики консолидированы 34 → 12)