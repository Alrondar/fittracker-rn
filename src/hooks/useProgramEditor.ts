...
export function useProgramEditor(programId: string, userId: string | null) {
  ...
  // ✅ Состояние поиска/списка/сортировки пикера УДАЛЕНО: пикер теперь самодостаточен
  //    (useExercises внутри ExercisePickerSheet). Прямой select('*') в хуке убран —
  //    это закрывает нарушение «тяжёлые данные в список» и «from() вне сервиса».
...
const handleStartProgram = async () => {
  ...
  const toggleEditMode = async () => {
    if (editMode) {
      ...
      //    RPC создаёт копии с uuid без 'user_', поэтому старая проверка принимала
      //    СОБСТВЕННУЮ копию за сид и повторно звала копирование → duplicate key / дубль.
  ...
  // Переупорядочить фазы (drag & drop). Экспорт оставлен для обратной совместимости;
  // на данный момент UI переупорядочивает фазы стрелками (movePhase), не драгом.
...
const saveProgram = async () => {
  ...
  try {
    ...
    // ✅ ПРОВЕРКА: editedProgram должен быть копией пользователя, а не сидом
    ...
    // ✅ ИСПРАВЛЕНИЕ: проверить КАЖДЫЙ промис на ошибку
    ...
    // ✅ ИСПРАВЛЕНИЕ: принудительный рефetch без кэша
