...
export function NutritionAddModal({ visible, onClose, editingLog }: NutritionAddModalProps) {
  ...
  useEffect(() => {
    if (editingLog) {
      setMealType(editingLog.meal_type);
      setCalories(String(editingLog.calories ?? ''));
      setProteins(String(editingLog.proteins ?? ''));
      setFats(String(editingLog.fats ?? ''));
      setCarbs(String(editingLog.carbs ?? ''));
      setWater(String(editingLog.water_ml ?? ''));
    }
  }, [editingLog]);
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isEditing && editingLog) {
        await profileService.updateNutritionLog(editingLog.id, data);
      } else {
        await profileService.saveNutritionLog(userId, data);
      }
    },
  ...
  return (
    <Modal transparent visible={visible} animationType="slide" onRequestClose={onClose}>
      <SheetShell
        isModal
        title={isEditing ? 'Редактировать приём пищи' : 'Добавить приём пищи'}
        onClose={onClose}
      >
        <View
          style={{
        ...
        >
          ...
          <AppButton
            title={isEditing ? 'Сохранить изменения' : 'Сохранить'}
            variant="primary"
            onPress={handleSave}
