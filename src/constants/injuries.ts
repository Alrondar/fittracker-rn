...
export type BodyPart = 'knee' | 'shoulder' | 'elbow' | 'wrist' | 'back' | 'neck' | 'hip' | 'ankle';

export type InjuryType = 'strain' | 'sprain' | 'pain' | 'inflammation' | 'fracture' | 'other';
...
/**
 * FD12-6: канон совпадения противопоказания с активной травмой (hard-exclusion,
 * уровень 1). Совпадение по зоне — всегда; совпадение по типу — только для
 * осмысленных типов: 'other'/NULL — сборная солянка (в БД 62 из 279 строк
 * warnings), её матчинг по типу сквозь зоны ложно исключал упражнения.
 * Единый предикат для alternatives (ENG-5) и разминки (ARCH-8) — не дублировать.
 * (Не путать с deprecated matchesContraindication — легаси по строкам text.)
 */
export function contraindicationMatchesInjury(
  contra: { body_part: string; injury_type?: string | null },
  injury: UserInjury
): boolean {
  if (contra.body_part === injury.body_part) return true;
  const type = contra.injury_type;
  return type != null && type !== 'other' && injury.injury_type === type;
}
...
export function matchesContraindication(
  ...
  return exerciseInjuries.some((contraindication) => {
    ...
    return (
      bpKeywords.some((kw) => lower.includes(kw)) || itKeywords.some((kw) => lower.includes(kw))
    );
  });
}
