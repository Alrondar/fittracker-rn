...
const DETAIL_FIELDS =
  'id, name, technique, primary_muscles, secondary_muscles, benefits, risks, settings, category, media_url, can_be_activation';
export async function getExerciseById(id: string): Promise<ExerciseDetail | null> {
  ...
  if (error) throw error;
  if (!data) return null;
  const ref = (await getExerciseReferenceData([id]))[id] ?? {
    equipment: [],
    injuries: [],
    alternativeIds: [],
  };
  return {
    ...
    injuries: ref.injuries,
    alternatives: ref.alternativeIds,
...
export async function getExercisesByIds(ids: string[]): Promise<ExerciseListItem[]> {
  ...
  return (data ?? []).map((row) => ({
    ...
    name: row.name,
    primary_muscles: row.primary_muscles ?? [],
    equipment: refs[row.id]?.equipment ?? [],
  }));
}
...
interface RecordLogRowWithWarmup {
  weight_kg: number | null;
  reps: number | null;
  reps_left?: number | null;
  reps_right?: number | null;
...
export async function getExerciseRecords(
  ...
  const { data, error } = await supabase
    ...
    .select(
      'id, workouts!inner ( id, finished_at, started_at ), workout_logs ( weight_kg, reps, reps_left, reps_right, is_warmup )'
    )
  ...
  let maxWeight: number | null = null,
    repsAtMaxWeight = 0,
    maxReps: number | null = null,
    bestE1RM: number | null = null;
  let totalVolume = 0,
    totalSets = 0,
    workoutCount = 0,
    lastPerformedAt: string | null = null;
