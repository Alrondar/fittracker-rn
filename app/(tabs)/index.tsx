...
export default function DashboardScreen() {
  ...
  const [
    ...
    setNutritionModalVisible,
  ] = useState(false);

  // NUTRI-2:
  // модалка списка записей + редактирование.
  const [
    nutritionLogListVisible,
    setNutritionLogListVisible,
  ] = useState(false);

  const [
    editingNutritionLog,
    setEditingNutritionLog,
  ] =
    useState<NutritionLog | null>(
      null,
    );
  ...
  const handleStartWorkout =
    useCallback(() => {
      if (
        ...
        router.push(
          `/workout/create?programId=${data.activeProgram.programId}`,
        );
      }
    }, [
      data?.activeProgram,
