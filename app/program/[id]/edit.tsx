...
export default function ProgramEditScreen() {
  ...
  // ✅ useCallback перемещён ВЫШЕ раннего возврата (исправление react-hooks/rules-of-hooks)
  const getIntensityInfo = useCallback(
    (intensity: string) => {
      switch (intensity) {
        case 'high':
          return { label: 'Высокая', color: colors.error, icon: null };
        case 'medium':
          return { label: 'Средняя', color: colors.warning, icon: null };
        case 'low':
          return { label: 'Низкая', color: colors.success, icon: null };
        default:
          return { label: intensity, color: colors.textSecondary, icon: null };
      }
    },
    [colors],
  );
  ...
  return (
    ...
    >
          ...
          {phases.map((phase, phaseIndex) => (
            <PhaseCard
              ...
              onExerciseSettings={(day, exerciseIndex) => {
                setSelectedDay(day);
                setSelectedDayIndex(day.day_number - 1); // Упрощённо, лучше использовать flatIndex
      ...
      {/* Футер с кнопкой Отмена (дублирует header для удобства) */}
      <View
        style={[
          commonStyles.footer,
